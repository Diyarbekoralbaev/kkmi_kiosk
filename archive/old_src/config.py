...
# --- Stub classes for removed providers (kept to satisfy legacy pipeline imports) ---
# These are NEVER instantiated — corresponding YAML sections are removed.
class LocalProviderConfig(BaseModel):
    """Stub: local provider was removed in cleanup. Kept for import compatibility."""
...
class DeepgramProviderConfig(BaseModel):
    """Stub: deepgram provider was removed in cleanup. Kept for import compatibility."""
    model_config = ConfigDict(extra="allow")
    ...
    voice_agent_base_url: str = Field(default="wss://agent.deepgram.com/v1/agent/converse")
    farewell_hangup_delay_sec: Optional[float] = None


class OpenAIRealtimeProviderConfig(BaseModel):
    """Stub: openai_realtime provider was removed in cleanup. Kept for import compatibility."""
    model_config = ConfigDict(extra="allow")
    enabled: bool = Field(default=False)
    api_key: Optional[str] = None
...
class MCPServerConfig(BaseModel):
    ...
    framing: str = Field(default="content-length")  # "content-length" (AVA servers) or "newline" (MCP SDK servers)
