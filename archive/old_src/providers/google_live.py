...
class GoogleLiveProvider(AIProviderInterface):
    ...
    async def _send_setup(self, context: Optional[Dict[str, Any]]) -> None:
        ...
        # Detect Gemini 3.1 Live models
        _effective_model = self._normalize_model_name(self.config.llm_model)
        _is_gemini31_live = "3.1" in _effective_model and "live" in _effective_model
        self._is_gemini31_live = _is_gemini31_live
        logger.info("Model detection", call_id=self._call_id, model=_effective_model, is_gemini31=_is_gemini31_live)

        # Build generation config
        if _is_gemini31_live:
            # Gemini 3.1: snake_case speechConfig, no maxOutputTokens (SDK wire format)
            generation_config = {
                "responseModalities": response_modalities,
                "speechConfig": {
                    "voice_config": {
                        "prebuilt_voice_config": {
                            "voice_name": self.config.tts_voice_name or "Aoede"
                        }
                    }
                },
            ...
            # Gemini 2.5: camelCase, full config
            generation_config = {
                "responseModalities": response_modalities,
                "speechConfig": {
                    "voiceConfig": {
        ...
        if system_prompt:
            ...
            si = {"parts": [{"text": system_prompt}]}
            if _is_gemini31_live:
                si["role"] = "user"  # SDK sends role: "user" for 3.1
            setup_msg["setup"]["systemInstruction"] = si
            logger.info("systemInstruction set", call_id=self._call_id, length=len(system_prompt), has_role=_is_gemini31_live)

        # Merge raw tool declarations (used by the kiosk UI — skips the
        # full engine ToolAdapter and uses the on_event hook instead).
        raw_tools = context.get("raw_tools", []) if context else []
        if raw_tools:
            if tools:
                tools[0].setdefault("functionDeclarations", []).extend(raw_tools)
            else:
                tools = [{"functionDeclarations": list(raw_tools)}]
    ...
    async def _send_greeting(self) -> None:
        ...
        _is_31 = getattr(self, '_is_gemini31_live', False)
        if _is_31:
            # Gemini 3.1: use realtime_input with text field (clientContent causes 1007)
            greeting_msg = {
                "realtime_input": {
                    "text": f"Please greet the caller with the following message: {greeting}"
                }
            }
            logger.info("Sending greeting via realtime_input.text (3.1)", call_id=self._call_id)
        else:
            # Gemini 2.5: use clientContent (validated Golden Baseline pattern)
            greeting_msg = {
                "clientContent": {
                    "turns": [
                        {
    ...
    async def send_text_turn(self, text: str) -> None:
        """
        Send a plain text user turn to Gemini Live (e.g. screen context signals
        from the kiosk UI). Handles both Gemini 3.1 (realtime_input.text) and
        Gemini 2.5 (clientContent) paths.
        """
        ...
        try:
            ...
            logger.info(
                "Google Live text turn sent",
                call_id=self._call_id,
                text=text[:80],
            )
        except Exception:
            logger.debug(
                "Google Live text turn failed",
                call_id=self._call_id,
                exc_info=True,
            )
    ...
    async def send_audio(self, audio_chunk: bytes, sample_rate: int = 8000, encoding: str = "ulaw") -> None:
        ...
        """
        if not self.websocket or not self._setup_complete or self._closing:
            if not hasattr(self, '_send_audio_skip_logged'):
                logger.info("send_audio SKIPPED", call_id=self._call_id, has_ws=bool(self.websocket), setup_complete=self._setup_complete, closing=self._closing)
                self._send_audio_skip_logged = True
            return
        ...
        if self._send_audio_count == 1:
            logger.info("send_audio FIRST CALL", call_id=self._call_id, chunk_bytes=len(audio_chunk), sample_rate=sample_rate, encoding=encoding)
        elif self._send_audio_count % 500 == 0:
            logger.info("send_audio progress", call_id=self._call_id, total_calls=self._send_audio_count)
        ...
        try:
            ...
            while len(self._input_buffer) >= chunk_size:
                ...
                # Send realtime input
                # Gemini 3.1 requires snake_case top-level key; 2.5 uses camelCase
                _is_31 = getattr(self, '_is_gemini31_live', False)
                if _is_31:
                    message = {
                        "realtime_input": {
                ...
                if self._send_audio_count == 1:
                    logger.info("send_audio FIRST MESSAGE", call_id=self._call_id, key="realtime_input" if _is_31 else "realtimeInput", rate=provider_rate, chunk_bytes=len(chunk_to_send))
    ...
    async def _handle_server_content(self, data: Dict[str, Any]) -> None:
        ...
        if input_transcription:
            ...
            if text:
                # Stream partial to any listener (kiosk UI, etc.)
                if self.on_event and self._call_id:
                    try:
                        merged_preview, _ = _merge_transcription_fragment(
                            self._input_transcription_buffer,
                            text,
                            self._last_input_transcription_fragment,
                        )
        ...
        if output_transcription:
            ...
            if text:
                ...
                # Stream partial to any listener (kiosk UI)
                if self.on_event and self._call_id:
                    try:
                        merged_preview, _ = _merge_transcription_fragment(
                            self._output_transcription_buffer,
                ...
                if self.on_event and self._call_id:
                    try:
                        await self.on_event({
                            ...
                            "call_id": self._call_id,
                            "speaker": "user",
                            "text": self._input_transcription_buffer,
                            "final": True,
                        })
                    except Exception:
                        pass
    ...
    async def _handle_tool_call(self, data: Dict[str, Any]) -> None:
        ...
        # ---- Kiosk UI raw tool handler (pre-adapter) -----------------
        # The kiosk session registers raw function declarations directly
        # (no engine ToolAdapter). Handle those here and exit early.
        function_calls_raw = tool_call.get("functionCalls", [])
        kiosk_handled_any = False
        for fc in list(function_calls_raw):
            if fc.get("name") != "navigate_to_screen":
                continue
            kiosk_handled_any = True
            screen = str((fc.get("args") or {}).get("screen") or "home")
            if self.on_event:
                try:
                    await self.on_event({
                        "type": "KioskNavigate",
                    ...
                    logger.debug(
                        "Kiosk navigate event forward failed",
                        call_id=self._call_id,
                        exc_info=True,
                    )
            # Respond to Gemini so the model can continue speaking
            safe_result = {"status": "ok", "screen": screen}
            call_id = fc.get("id")
            use_vertex = getattr(self, '_vertex_active', getattr(self.config, 'use_vertex_ai', False))
            if use_vertex:
                func_response = {"name": "navigate_to_screen", "response": safe_result}
            else:
                func_response = {"id": call_id, "name": "navigate_to_screen", "response": safe_result}
            try:
                await self._send_message({
                    "toolResponse": {"functionResponses": [func_response]}
                })
                logger.info(
                    "Kiosk navigate_to_screen handled",
                    call_id=self._call_id,
                    screen=screen,
                )
            except Exception:
                logger.debug(
                    "Kiosk navigate tool response failed",
                    call_id=self._call_id,
                    exc_info=True,
                )

        # If all function calls in this message were kiosk tools, we're done.
        if kiosk_handled_any and all(
            fc.get("name") == "navigate_to_screen" for fc in function_calls_raw
        ):
            return
